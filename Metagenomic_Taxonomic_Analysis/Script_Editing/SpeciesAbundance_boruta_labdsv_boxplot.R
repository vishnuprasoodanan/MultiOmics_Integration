...
library(labdsv)
#---------------------------------------------------------------------------------------
df <- read.table("kraken2_species_readcount.txt", sep = "\t", header = T, row.names = 1)
...
df_relative <- as.data.frame(t(apply(df, 1, function(x) x/sum(x))))

df_relative[df_relative <= 0.0002] <- 0
zero_sum_cols <- sapply(df_relative, function(x) sum(x) == 0)
# Remove columns with sum zero
df_relative <- df_relative[, !zero_sum_cols]

# Identify rows with sum zero
zero_sum_rows <- rowSums(df_relative) == 0

# Remove rows with sum zero
df_ref <- df_relative[!zero_sum_rows, ]

# Merge the dataframes by row names
merged_df <- merge(df_ref, metadata, by = "row.names", all = TRUE)
...
write.table(merged_df, file = "Kraken2_relative_abundace.txt", quote = FALSE, sep = '\t')
#---------Boruta Analysis
...
write.table(merged_df_t, file = "Lefse_input.txt", sep = '\t', quote = FALSE)
